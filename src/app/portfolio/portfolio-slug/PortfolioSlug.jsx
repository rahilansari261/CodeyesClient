...
export default function PortfolioSlug() {
    ...
    return (
        <>
            <PortBanner image={data?.banner_image} />
            {/* <div className='p-3'>
                <img src={data.banner_image} alt={data?.name} className='border border-black rounded-[50px]' />
            </div> */}
                ...
                <div className='grid grid-cols-12 gap-5 my-10 items-end'>
                    ...
                    <div className='col-span-12 lg:col-span-3'>
                        <div className='space-y-2'>
                            <p><span className='text-lg text-[#151015] font-semibold'>Client:</span>&nbsp; {data?.client}</p>
                            <p><span className='text-lg text-[#151015] font-semibold'>Category:</span>&nbsp; ({data?.category?.name})</p>
                            <p><span className='text-lg text-[#151015] font-semibold'>Tag:</span>&nbsp; <span className='border border-black py-1 px-3 rounded-full'>{data?.tags.map((item) => item)}</span></p>
                            <p><span className='text-lg text-[#151015] font-semibold'>Date:</span>&nbsp; {moment(data?.createdAt).format("DD-MM-YYYY")}</p>
                        </div>
                    </div>
                </div>
