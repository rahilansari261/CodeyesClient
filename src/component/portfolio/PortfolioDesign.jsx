      ...
      <svg className="svg-distort">
        <defs>
          <filter id="displacement-filter" className="svg_filter">
            <feTurbulence type="fractalNoise" baseFrequency="0.02 0.05" numOctaves="5" result="warp" />
            <feGaussianBlur in="warp" stdDeviation="8" result="blurredWarp" />
            <feDisplacementMap in="SourceGraphic" in2="blurredWarp" scale="150" xChannelSelector="R" yChannelSelector="G" />
          </filter>
        </defs>
      </svg>
      <div className="grid md:grid-cols-2 lg:grid-cols-4 ">
        {blogPosts &&
          ...
          blogPosts.map((item, index) => (
            <div className="col-span-1 border border-black relative cursor-pointer group rounded-[40px] overflow-hidden port_img" key={index} onMouseMove={(e) => handleMouseMove(e, e.currentTarget)}>
              <Link to={`/portfolio-slug/${item?.id}`}>
                <img src={`${IMAGE_BASE_URL}/${item?.banner_image[0]}`} alt="banner-image" className="h-full w-full object-cover img_hover_effect" />
                <img src={`${IMAGE_BASE_URL}/${item?.banner_image[0]}`} alt="banner-image" className="h-full w-full object-cover img_hover_effect" />
                <div
                  style={{
